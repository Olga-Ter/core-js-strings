...
function removeFirstOccurrences(str, value) {
  let ans;
  if (!value || !str.match(new RegExp(value))) ans = str;
  else if (!str) ans = '';
  else
    ans = str
...
function countVowels(str) {
  ...
  let count = 0;
  [...str].forEach((element) => {
    if (vowels.join('').includes(element)) count += 1;
  });
  return count;
}
...
function findLongestWord(sentence) {
  ...
  if (sentence) {
    sentence.split(' ').forEach((element) => {
      if (element.length > ans.length) ans = element;
    });
  }
  return ans;
}
...
function reverseWords(str) {
  ...
  if (str) {
    ...
    arr.forEach((element, index) => {
      arr[index] = [...element].reverse().join('');
    });
    ans = arr.join(' ');
  }
  return ans;
...
function invertCase(str) {
  ...
  [...str].forEach((element) => {
    ...
    if (element === char) newstr += element.toUpperCase();
    else newstr += element.toLowerCase();
  });
  return newstr;
}
...
function encodeToRot13(str) {
  ...
  let ans = '';
  [...str].forEach((element) => {
    if (element === ' ' || sign.includes(element)) ans += element;
    else ans += rot13.at(alpha.indexOf(element));
  });
  return ans;
}
